...
if menu == "Register Company":
    ...
    if st.button("Save Company"):
        try:
            ...
            if name_count > 0:
                st.error(f"⚠️ A company with name '{company_name}' already exists!")
            elif full_count > 0:
                st.error(f"⚠️ A company with full name '{full_name}' already exists!")
            else:
                cursor.execute("""
    ...
    try:
        ...
        if companies:
            ...
            with st.form("update_form"):
                ...
                new_company_responsible = st.selectbox("Company Responsible", ["None", "Nefeli", "Aggelos"],
                                                      index=(["None","Nefeli","Aggelos"].index(row[2]) if row[2] in ["None","Nefeli","Aggelos"] else 0))
                new_company_project_responsible = st.selectbox("Company Project Responsible", ["None", "Nefeli", "Aggelos"],
                                                      index=(["None","Nefeli","Aggelos"].index(row[3]) if row[3] in ["None","Nefeli","Aggelos"] else 0))
    ...
    try:
        ...
        if companies:
            ...
            with st.form("project_form"):
                ...
                project_responsible = st.selectbox("Project Responsible", ["None", "Nefeli", "Aggelos"])

                submitted = st.form_submit_button("Save Project")
                if submitted:
                    # --- Validation rules ---
                    if set_review and not set_received:
                        st.error("⚠️ You cannot set Data Review without setting Data Received first.")
                    elif set_report and not set_review:
                        st.error("⚠️ You cannot set Report Date without setting Data Review first.")
                    elif set_received and not set_start:
                        st.error("⚠️ You cannot set Data Received without setting Start Date first.")
                    else:
                        ...
                        if count > 0:
                            ...
                            st.success(f"✏️ Project for company '{company_choice}' updated successfully!")
                        else:
                            cursor.execute("""
    ...
    try:
        ...
        base_query = """
            SELECT c.company_name, c.full_name, c.sector, c.company_responsible, c.project_responsible,
                   ...
                   p.invoice_amount, p.is_paid,
                   p.project_responsible,
                   COALESCE(NULLIF(p.project_responsible, 'None'), c.project_responsible) AS final_responsible
            FROM companies c
        ...
        if rows:
            df = pd.DataFrame(rows, columns=[
                ...
                "Invoice Amount", "Paid", "Project Responsible", "Final Responsible"
            ])
